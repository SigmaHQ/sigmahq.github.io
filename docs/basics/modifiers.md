...
The windash modifier will convert any provided command-line arguments or flags to use `-`, as well as `/`, `–`, `—`, and `―`.
...
Many tools, including PowerShell, will not only accept a normal hyphen, but other similar looking dashes like `–`, `—`, and `―` as well.
